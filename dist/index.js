...
_postAddChecks = function _postAddChecks(timeline, child) {
  ...
  return (_globals.ScrollTrigger || _missingPlugin()) && _globals.ScrollTrigger.create(trigger, animation);
},
...
toArray = function toArray(value, scope, leaveStrings) {
  ...
  value = toArray(value)[0] || _warn() || {};
  return function (v) {
    var el = value.current || value.nativeElement || value;
    return toArray(v, el.querySelectorAll ? el : el === value ? _warn() || _doc$1.createElement("div") : value);
  };
},
...
var _gsap = {
  ...
  registerEffect: function registerEffect(_ref3) {
    ...
    (plugins || "").split(",").forEach(function (pluginName) {
      return pluginName && !_plugins[pluginName] && !_globals[pluginName] && _warn();
    });
...
class Shape {
    constructor(pos, className, cssText) {
        this.x = pos.x;
        this.y = pos.y;
...
class Circle extends Shape {
    constructor(pos, size, borderWidth, borderColor) {
        const cssText = `
    --width: ${size}px;
    --height: ${size}px;
